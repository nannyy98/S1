...
SQLite + локальные картинки в static/uploads.
Совместимо с URL видами:
- /uploads/<file>        ← новый совместимый роут
- /static/uploads/<file> ← ваш старый роут (оставлен)
"""
import logging
import os
import sys
...
inventory_manager = InventoryManager(db)  # ← было не инициализировано

# Настройки загрузки файлов
UPLOAD_SUBDIR = 'static/uploads'
ALLOWED_EXTENSIONS = {'png', 'jpg', 'jpeg', 'gif', 'webp'}
MAX_FILE_SIZE = 16 * 1024 * 1024  # 16MB

app.config['UPLOAD_FOLDER'] = os.path.join(os.path.dirname(__file__), UPLOAD_SUBDIR)
...
def _uploads_relative(filename: str) -> str:
    """/uploads/<filename> — удобный и короткий URL для клиентов/бота."""
    return f"/uploads/{filename}"

def _uploads_static_relative(filename: str) -> str:
    """/static/uploads/<filename> — сохраняем совместимость со старыми путями."""
    return f"/{UPLOAD_SUBDIR}/{filename}"

def _absolute(url_root: str, path: str) -> str:
    return url_root.rstrip('/') + path

def save_local(file_storage) -> str:
    """
    Сохраняет файл в UPLOAD_FOLDER и возвращает ПОЛНЫЙ публичный URL
    вида https://<host>/uploads/<uuid>.<ext>
    """
    ...
    if ext not in ALLOWED_EXTENSIONS:
        raise ValueError("Недопустимый формат файла (png/jpg/jpeg/webp/gif)")

    fname = f"{uuid.uuid4()}.{ext}"
    disk_path = os.path.join(app.config['UPLOAD_FOLDER'], fname)
    os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)
    file_storage.save(disk_path)

    # Возвращаем новый короткий путь /uploads/...
    rel = _uploads_relative(fname)
    return _absolute(request.url_root, rel)
...
def add_product():
    if request.method == 'POST':
        ...
        # изображение: приоритет — загруженный файл; fallback — ссылка из формы
        image_url = ''
        file = request.files.get('image_file')
        if file and file.filename and allowed_file(file.filename):
            try:
                image_url = save_local(file)  # вернёт https://…/uploads/<uuid>.<ext>
...
# ========= Раздача файлов загрузок =========
# Новый короткий роут (исправляет 404 по /uploads/<file>)
...
def serve_uploads(filename):
    return send_from_directory(
        ...
        filename,
        as_attachment=False,
        max_age=60 * 60 * 24 * 365,  # кэш 1 год
        conditional=True
    )

# Старый роут совместимости (оставляем)
@app.route('/static/uploads/<filename>')
def uploaded_file(filename):
    return send_from_directory(app.config['UPLOAD_FOLDER'], filename)

# ========= Категории, клиенты, CRM, отчёты, склад, посты и т.п. =========
# (ниже — ваш исходный код без изменений, за исключением единичных правок сохранения изображений)
...
def crm_page():
    try:
        ...
        return render_template('crm.html', segments=segments, at_risk_customers=at_risk_customers)
    except ImportError:
        ...
        for r in rows:
            ...
            days = 999
            try:
                days = (now - _dt.fromisoformat(last_date)).days if last_date else 999
            except Exception:
                pass
...
def scheduled_posts():
    try:
        ...
        stats = db.execute_query('''
            ...
            LIMIT 10
        ''')
        return render_template('scheduled_posts.html', posts=posts or [], stats=stats or [])
    except Exception as e:
    ...
    try:
        ...
        if target_audience == 'channel':
            if image_url and len(message_text) <= 1024:
                ...
                _ = telegram_bot.send_photo(posts_manager.channel_id, image_url, '', None)
            ...
            for recipient in recipients:
                try:
                    ...
                    if image_url and len(message_text) <= 1024:
                        ...
                        _ = telegram_bot.send_photo(telegram_id, image_url, '', None)
...
def test_channel_post():
    ...
    try:
        ...
        flash('✅ Тестовый пост с изображением отправлен в канал!' if result and result.get('ok') else f'Ошибка отправки в канал: {result}')
    except Exception as e:
...
def toggle_post_status():
    ...
    result = db.execute_query('UPDATE scheduled_posts SET is_active = ? WHERE id = ?', (new_status, post_id))
    if result and result > 0:
        telegram_bot.trigger_bot_data_reload()
        flash(f'Пост {"включен" if new_status else "выключен"}!')
    else:
...
# ----- Обновления бота / статусы заказов -----
...
def toggle_product(product_id):
    ...
    new_status = 0 if current_status else 1
    result = db.execute_query('UPDATE products SET is_active = ? WHERE id = ?', (new_status, product_id))
    if result and result > 0:
        telegram_bot.trigger_bot_data_reload()
        flash(f'Товар {"активирован" if new_status else "скрыт"}!')
    else:
...
def notify_new_product():
    ...
    try:
        ...
        flash(f'Уведомление о товаре "{product[1]}" отправлено в канал!' if result and result.get('ok') else f'Ошибка отправки в канал: {result}')
    except Exception as e:
...
def send_broadcast():
    ...
    try:
        if target_audience == 'all':
            recipients = db.execute_query('SELECT telegram_id, name, language FROM users WHERE is_admin = 0')
...
def toggle_category_status():
    ...
    current_status = int(request.form['current_status'])
    new_status = 0 if current_status else 1
    result = db.execute_query('UPDATE categories SET is_active = ? WHERE id = ?', (new_status, category_id))
    if result and result > 0:
        telegram_bot.trigger_bot_data_reload()
        flash(f'Категория {"активирована" if new_status else "скрыта"}!')
    else:
...
def delete_category():
    ...
    if not category_id:
        ...
        return redirect(url_for('categories'))
    try:
        products = db.execute_query('SELECT COUNT(*) FROM products WHERE category_id = ?', (category_id,))
        if products and products[0][0] > 0:
            flash(f'Невозможно удалить категорию: в ней есть {products[0][0]} товар(ов)')
            return redirect(url_for('categories'))
        result = db.execute_query('DELETE FROM categories WHERE id = ?', (category_id,))
        if result:
            telegram_bot.trigger_bot_data_reload()
...
def reload_bot_data():
    try:
        flash('Сигнал обновления отправлен боту!' if telegram_bot.trigger_bot_data_reload() else 'Ошибка отправки сигнала обновления')
    except Exception as e:
...
def force_reload_bot():
    try:
        ...
        reload_message = "🔄 <b>ПРИНУДИТЕЛЬНОЕ ОБНОВЛЕНИЕ</b>\n\n✅ Все данные перезагружены\n✅ Кэш очищен\n✅ Автопосты перезапущены\n\n"
...
def test_telegram():
    try:
        if telegram_bot.test_connection():
            return jsonify({'success': True, 'bot_name': 'Shop Bot'})
        else:
            return jsonify({'success': False, 'error': 'Не удалось подключиться к Telegram API'})
    except Exception as e:
...
def inventory_page():
    try:
        ...
        ''') or []
        low_stock = {'critical': [row for row in low_stock_rows if row[2] == 0],
        ...
        abc_analysis = {'total_value': stock_cost, 'categories': {'A': [], 'B': [], 'C': []}}
        return render_template('inventory.html',
            inventory_summary=inventory_summary, low_stock=low_stock, abc_analysis=abc_analysis
        )
    except Exception as e:
